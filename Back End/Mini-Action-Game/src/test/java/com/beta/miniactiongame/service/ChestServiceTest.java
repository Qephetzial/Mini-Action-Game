...
class ChestServiceTest {
    ...
    @Test
    void openChest() {
        List<Items> item = chestService.openChest(800, 950, 990, 1000, false);
        assertNotNull(item);
    }

    @Test
    void getCommonItem() {
        List<Items> item = chestService.openChest(1000,0,0,0,false);
        assertEquals(Rarity.COMMON, item.get(0).getRarity());
    }

    @Test
    void getUnCommonItem() {
        List<Items> item = chestService.openChest(0,1000,0,0,false);
        assertEquals(Rarity.UNCOMMON, item.get(0).getRarity());
    }

    @Test
    void getRareItem() {
        List<Items> item = chestService.openChest(0,0,1000,0,false);
        assertEquals(Rarity.RARE, item.get(0).getRarity());
    }

    @Test
    void getEpicItem() {
        List<Items> item = chestService.openChest(0,0,0,1000,false);
        assertEquals(Rarity.EPIC, item.get(0).getRarity());
    }

    @Test
    void getLegendaryItem() {
        List<Items> item = chestService.openChest(0,0,0,0,false);
